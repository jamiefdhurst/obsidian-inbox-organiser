...
describe('OrganiserModal', () => {
  ...
  beforeEach(() => {
    ...
    fixtureFiles = [new TFile(), new TFile()];
    fixtureFiles[0].name = 'test_1.md';
    fixtureFiles[1].name = 'test_2.md';
    fixtureFolders = [new TFolder(), new TFolder(), new TFolder()];
  ...
  it('selects/deselects all files at once', async () => {
    ...
    expect(
      (tableEl.children[1].children[0].children[0].children[0] as HTMLInputElement).checked
    ).toEqual(true);
    expect(
      (tableEl.children[1].children[1].children[0].children[0] as HTMLInputElement).checked
    ).toEqual(true);
  ...
  it('moves a single file', async () => {
    ...
    const tableEl = sut.contentEl.find(`.${CLS_PREFIX}files table`);
    (tableEl.children[1].children[0].children[2].children[0] as HTMLSelectElement).value =
      '/Level One';
    tableEl.children[1].children[0].children[2].children[0].dispatchEvent(new Event('change'));
    await new Promise((r) => setTimeout(r, WAIT_TIME));
